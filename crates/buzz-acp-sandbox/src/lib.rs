...
const CHILD_COMMAND_TARGETS: &[(&str, &str)] = &[
    ("BUZZ_ACP_AGENT_COMMAND", "/run/buzz/bin/buzz-acp-agent"),
    ("BUZZ_ACP_MCP_COMMAND", "/run/buzz/bin/buzz-acp-mcp"),
];
...
pub struct SandboxConfig {
    ...
    pub child_command_binds: Vec<ChildCommandBind>,
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidBind(bind) => write!(
                f,
                "invalid BUZZ_SANDBOX_BIND entry {bind:?}; expected /source:ro, /source:rw, or /source=/target:ro with a target outside host home paths"
            ),
            ...
            Self::InvalidCwd(cwd) => write!(
                f,
                "invalid BUZZ_SANDBOX_CWD {cwd:?}; expected an absolute sandbox path outside host home paths"
            ),
...
impl SandboxConfig {
        ...
        F: Fn(&str) -> Option<String>,
    {
        ...
        if !is_safe_sandbox_path(&cwd) {
            return Err(SandboxError::InvalidCwd(path_arg(&cwd)));
        }
        let extra_binds = parse_bind_list(get("BUZZ_SANDBOX_BIND").as_deref().unwrap_or(""))?;
        let child_command_binds = child_command_binds(&get);
...
pub fn build_launch_plan(
    ...
    for bind in &config.child_command_binds {
        add_child_command_bind(&mut args, bind);
    }
...
fn add_child_command_bind(args: &mut Vec<String>, bind: &ChildCommandBind) {
    args.push("--ro-bind".to_string());
    args.push(path_arg(&bind.source));
    args.push(path_arg(&bind.target));
    args.push("--setenv".to_string());
    args.push(bind.env_key.clone());
    args.push(path_arg(&bind.target));
}
...
fn is_safe_sandbox_path(path: &Path) -> bool {
    if path
        .components()
    ...
    !path.starts_with("/home") || path.starts_with(SANDBOX_HOME)
}

fn child_command_binds<F>(get: &F) -> Vec<ChildCommandBind>
where
    F: Fn(&str) -> Option<String>,
{
    CHILD_COMMAND_TARGETS
        .iter()
        .filter_map(|(env_key, target)| {
            let value = get(env_key)?.trim().to_string();
            if value.is_empty() {
                return None;
            }
...
mod tests {
    ...
    #[test]
    fn absolute_child_commands_are_bound_and_rewritten_inside_sandbox() {
        let mut config = config_with(&[]);
        config.child_command_binds = vec![ChildCommandBind {
            env_key: "BUZZ_ACP_MCP_COMMAND".to_string(),
            source: PathBuf::from("/host/bin/buzz-dev-mcp"),
            target: PathBuf::from("/run/buzz/bin/buzz-acp-mcp"),
        }];
        ...
        assert_contains_triplet(
            &plan.args,
            "--ro-bind",
            "/host/bin/buzz-dev-mcp",
            "/run/buzz/bin/buzz-acp-mcp",
        );
        assert_contains_triplet(
            &plan.args,
            "--setenv",
    ...
    #[test]
    fn sandbox_cwd_rejects_host_home_paths() {
        let err = SandboxConfig::from_lookup(|key| match key {
            "HOME" => Some("/home/alice".to_string()),
            "PATH" => Some("/bin:/usr/bin".to_string()),
            "BUZZ_REAL_ACP" => Some("/bin/sh".to_string()),
            "BUZZ_SANDBOX_CWD" => Some("/home/alice/work/repo".to_string()),
            _ => None,
        })
        .expect_err("host home cwd should fail");
    ...
    fn bind_parser_rejects_malformed_inputs() {
        for raw in [
            ...
            "/source=:ro",
            "/home/alice/repo:rw",
            "/host/repo=/home/alice/repo:rw",
            "/host/repo=/workspace/../home/alice/repo:rw",
        ] {
            assert!(
