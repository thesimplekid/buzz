...
independently published crates. These shared transport and pairing crates live
in the `thesimplekid/buzz` fork; upstream does not yet include them. Pin an exact
commit from the fork:

```toml
[dependencies]
buzz-client = {
  git = "https://github.com/thesimplekid/buzz",
  rev = "<exact-commit>",
}
```
