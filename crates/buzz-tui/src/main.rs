...
use app::{App, ConfirmAction, Focus};
use clap::Parser;
...
fn managed_agent_runtime(
    agent: &crate::cli::ManagedAgentInfo,
    ...
    AgentRuntime {
        ...
        relay_url: Some(buzz_tui_client::relay_http_to_ws_url(&agent.relay_url)),
...
async fn run_app(
    ...
    while !app.should_quit {
        ...
        while let Ok(event) = live_rx.try_recv() {
            apply_live_event(app, event).await;
        }
        ...
        if event::poll(Duration::from_millis(150))? {
            if let Event::Key(key) = event::read()? {
                handle_key(app, session_config, key).await;
...
async fn apply_live_event(app: &mut App, event: LiveEvent) {
    match event {
        LiveEvent::Message(message) => {
            app.apply_live_message(message).await;
        }
...
async fn handle_key(app: &mut App, session_config: &SessionConfig, key: KeyEvent) {
