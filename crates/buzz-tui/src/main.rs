...
async fn run_app(
    ...
    while !app.should_quit {
        ...
        while let Ok(result) = palette_rx.try_recv() {
        ...
        // A cached channel paint requests its authoritative refresh here so
        // the switch itself never blocks on the network.
...
async fn handle_key(
    ...
    match key.code {
        ...
        KeyCode::Char('s') if app.focus == Focus::Sidebar => {
            app.toggle_selected_channel_sort().await;
        }
