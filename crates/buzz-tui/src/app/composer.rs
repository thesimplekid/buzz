    ...
    fn completion_matches(&self, kind: CompletionKind, query: &str) -> Vec<CompletionItem> {
        ...
        let mut items: Vec<(CompletionItem, bool)> = match kind {
            CompletionKind::Mention => {
                ...
                let active_channel_id = self.active_channel().map(|channel| channel.id);
                ...
                for agent in &self.relay_agents {
                    if relay_agent_is_reachable(
                        agent,
                        own_pubkey.as_deref(),
                        active_channel_id.as_deref(),
                        &channel_member_pubkeys,
                    ) && nostr_pubkey_uri(&agent.pubkey).is_some()
                    {
                ...
                let mut people = candidates
                    ...
                    .map(|(label, pubkey, source)| {
                        ...
                        (
                            CompletionItem {
                                display,
                                insert: mention_insert(label, pubkey),
                            },
        ...
        items.retain(|item| seen_inserts.insert(item.insert.clone()));
        items.truncate(8);
        items
    }
...
fn relay_agent_is_reachable(
    agent: &crate::client::RelayAgentInfo,
    own_pubkey: Option<&str>,
    active_channel_id: Option<&str>,
    channel_member_pubkeys: &BTreeSet<&str>,
) -> bool {
    if channel_member_pubkeys.contains(agent.pubkey.as_str()) {
        return true;
    }
    if active_channel_id.is_some_and(|channel_id| {
        agent.channel_ids.iter().any(|id| id == channel_id)
            || agent.channels.iter().any(|id| id == channel_id)
    }) {
        return true;
    }
    if agent.respond_to.as_deref() == Some("anyone") {
        return true;
    }
    let Some(own_pubkey) = own_pubkey else {
        return false;
    };
    if agent.owner_pubkey.as_deref() == Some(own_pubkey) {
        return true;
    }
    agent.respond_to.as_deref() == Some("allowlist")
        && agent
            .respond_to_allowlist
            .iter()
            .any(|pubkey| pubkey == own_pubkey)
}
