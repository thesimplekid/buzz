use super::{
    clamp_index, nostr_pubkey_uri, parse_agent_allowlist, short_id, AgentCreateField,
...
impl App {
    ...
    pub async fn focus_agents(&mut self) {
        ...
        self.refresh_requested_relay_agents().await;
        self.refresh_agent_turn_metrics().await;
    }

    /// Populate the relay directory after the Agents panel becomes visible.
    ///
    /// Visibility and keyboard focus are synchronous UI state changes, so they
    /// set a one-shot request that the next async UI boundary consumes without
    /// repeatedly querying the relay on every frame.
    pub async fn refresh_requested_relay_agents(&mut self) {
        if !std::mem::take(&mut self.relay_agents_refresh_requested) {
            return;
        }
    ...
    /// Refresh decrypted NIP-AM turn metrics for agents owned by this key.
    pub(super) async fn refresh_agent_turn_metrics(&mut self) {
        let Ok(client) = self.native_relay_client() else {
            return;
        };
        if let Ok(metrics) = client.agent_turn_metrics(100).await {
    ...
    pub(super) async fn toggle_selected_agent(&mut self) {
        ...
        if self.acp.position_of(&id).is_none() {
            self.status =
                format!("{label} is a relay directory agent; local controls are disabled");
            return;
        }
    ...
    pub fn agent_entries(&self) -> Vec<AgentListEntry<'_>> {
        let mut entries = self
            ...
            .collect::<Vec<_>>();
        entries.extend(self.relay_directory_agents().map(AgentListEntry::Relay));
        entries
    }

    pub fn agent_display_entries(&self) -> Vec<AgentDisplayEntry<'_>> {
        let local_agents = self.acp.agents().collect::<Vec<_>>();
        let relay_agents = self.relay_directory_agents().collect::<Vec<_>>();
        let mut entries = Vec::with_capacity(local_agents.len() + relay_agents.len() + 4);
        ...
        entries.push(AgentDisplayEntry::Header("Relay directory"));
        if relay_agents.is_empty() {
            entries.push(AgentDisplayEntry::Empty("No relay-only agents discovered"));
        } else {
            entries.extend(
                relay_agents
                    .into_iter()
    ...
    pub async fn refresh_selected_agent_log(&mut self) {
        ...
        if !managed {
            ...
            let mut text = if let Some(relay_agent) = self
                ...
                .cloned()
            {
                relay_agent_log_text(&relay_agent)
            } else {
                format!(
                    "Runtime: {label}\nCommand: {} {}\nStatus: {:?}",
                    command,
    ...
    pub(super) async fn refresh_agent_statuses(&mut self) {
        self.relay_agents_refresh_requested = false;
        for agent in self.managed_agent_store.infos() {
            self.sync_managed_agent(&agent);
        }
        match self.list_relay_agents_native().await {
            Ok(mut agents) => {
    ...
    fn relay_directory_agents(&self) -> impl Iterator<Item = &RelayAgentInfo> {
        let local_pubkeys = self
            .acp
            .agents()
            .map(|agent| agent.runtime.id.as_str())
            .collect::<BTreeSet<_>>();
        self.relay_agents
            .iter()
            .filter(move |agent| !local_pubkeys.contains(agent.pubkey.as_str()))
    }
...
fn relay_agent_log_text(agent: &RelayAgentInfo) -> String {
    let mut text = format!(
        "Relay agent: {}\nkey: {}\nstatus: {}\ntype: {}",
        agent.name,
    ...
    text.push_str("\n\nRelay directory agents are read-only here. @ mentions the agent; local runtime, log, autostart, delete, and memory actions are disabled.");
    text
}
