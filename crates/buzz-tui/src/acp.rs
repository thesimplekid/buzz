...
/// Environment marker stamped on every spawned harness so orphans from a
/// dead TUI process can be identified and reaped on the next launch.
const TUI_AGENT_MARKER_ENV: &str = "BUZZ_TUI_AGENT";

/// Kill agent harness process groups left behind by a previous TUI process.
/// Spawned harnesses carry `BUZZ_TUI_AGENT=<tui-pid>`; when that TUI pid no
/// longer exists the harness is orphaned. Linux-only (`/proc` scan); a no-op
/// elsewhere. Returns the number of process groups reaped.
#[cfg(target_os = "linux")]
pub fn reap_stale_agents() -> usize {
    let own_pid = std::process::id();
    let Ok(entries) = std::fs::read_dir("/proc") else {
        return 0;
    };
    let mut reaped_groups = std::collections::BTreeSet::new();
    for entry in entries.flatten() {
        ...
        let Some(owner_pid) = environ.split(|byte| *byte == 0).find_map(|variable| {
            std::str::from_utf8(variable)
                .ok()?
                .strip_prefix("BUZZ_TUI_AGENT=")?
                .parse::<u32>()
                .ok()
        }) else {
            continue;
        };
        if owner_pid == own_pid || std::path::Path::new(&format!("/proc/{owner_pid}")).exists() {
            continue;
        }
        let Some(pgid) = linux_process_group_of(pid) else {
            continue;
        };
        if reaped_groups.contains(&pgid) || pgid == own_pid {
            continue;
        }
        if kill_process_group(pgid) {
            reaped_groups.insert(pgid);
        }
    }
    reaped_groups.len()
}
...
/// The process group id from `/proc/<pid>/stat` (field after state and ppid).
#[cfg(target_os = "linux")]
fn linux_process_group_of(pid: u32) -> Option<u32> {
...
pub struct AgentRuntime {
    ...
    /// Normalized thinking effort (`none|minimal|low|medium|high|xhigh|max`),
    /// forwarded to harnesses via their thinking-effort env vars.
    pub thinking_effort: Option<String>,
    /// Normalized max output tokens forwarded via harness env vars.
...
impl AcpSupervisor {
    pub fn update_default_credentials(
        ...
        self.default_auth_tag = auth_tag;
    }
    ...
    pub fn start(&mut self, id: &str) -> Result<(), AcpError> {
        ...
        let launch = build_agent_launch(AgentLaunchInput {
            acp_binary,
            relay_url,
            private_key,
            auth_tag,
        ...
        // Ownership marker: lets a later TUI instance identify and reap
        // harnesses whose supervising TUI process died.
        command.env(TUI_AGENT_MARKER_ENV, std::process::id().to_string());
...
fn build_agent_launch(input: AgentLaunchInput) -> AgentLaunch {
    let AgentLaunchInput {
        acp_binary,
        relay_url,
        private_key,
        auth_tag,
    ...
    if runtime.managed {
        if let Some(repos_dir) = runtime.repos_dir.filter(|path| !path.trim().is_empty()) {
            env.insert("BUZZ_REPOS_DIR".to_string(), repos_dir.clone());
            env.insert("REPOS".to_string(), repos_dir);
        }
    }
    // Normalized config forwarded through the env-var contract each harness
    // reads (buzz-agent BUZZ_AGENT_*, Goose GOOSE_*); other harnesses ignore
    // the extra variables.
    if let Some(effort) = runtime
        .thinking_effort
        .filter(|value| !value.trim().is_empty())
    {
        env.insert("BUZZ_AGENT_THINKING_EFFORT".to_string(), effort.clone());
        env.insert("GOOSE_THINKING_EFFORT".to_string(), effort);
    }
...
mod tests {
    ...
    fn agent_launch_passes_credentials_through_env_only() {
        ...
        let launch = build_agent_launch(AgentLaunchInput {
            acp_binary: "buzz-acp".into(),
            relay_url: "ws://localhost:3000".into(),
            private_key: "nsec1secret".into(),
            auth_tag: Some("[\"auth\"]".into()),
            agent_owner: None,
            runtime,
            mcp_command: "buzz-dev-mcp".into(),
            respond_to: "owner-only".into(),
        });
    ...
    fn agent_launch_passes_owner_for_owner_scoped_modes() {
        ...
        let launch = build_agent_launch(AgentLaunchInput {
            ...
            private_key: "nsec1secret".into(),
            auth_tag: None,
            agent_owner: Some("owner-pubkey".into()),
            runtime,
            mcp_command: String::new(),
            respond_to: "owner-only".into(),
        });
    ...
    #[test]
    fn managed_agent_launch_passes_repo_directory_hints() {
        let runtime = AgentRuntime {
            id: "agent-pubkey".into(),
            label: "Review Bot".into(),
            relay_url: None,
            ...
            last_error: None,
            log_path: None,
            repos_dir: Some("/workspace/repos".into()),
        };
        ...
        assert_eq!(
            launch.env.get("BUZZ_REPOS_DIR").map(String::as_str),
            Some("/workspace/repos")
        );
        assert_eq!(
            launch.env.get("REPOS").map(String::as_str),
            Some("/workspace/repos")
        );
    }
