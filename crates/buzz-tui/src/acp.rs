...
mod tests {
    ...
    #[test]
    fn preserves_configured_runtime_order() {
    ...
    #[test]
    fn managed_agent_launch_subscribes_in_observe_mode() {
        let runtime = AgentRuntime {
        ...
        let subscribe_observe = launch
            .args
            .windows(2)
            .any(|window| window == ["--subscribe", "observe"]);
        let respond_to_anyone = launch
            .args
            .windows(2)
            .any(|window| window == ["--respond-to", "anyone"]);
        assert!(subscribe_observe);
