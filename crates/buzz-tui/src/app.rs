...
use crate::client::{
    ...
    AgentTurnMetric, Channel, ChannelMember, ChannelMessageSurface, ChannelPreferenceKind,
    ChannelSection, ChannelSections, Contact, ConversationKind, CreateChannelOptions,
    CreateIssueOptions, CreatePatchOptions, CreateRepoOptions, CustomEmojiEntry, GitIssue,
    GitPatch, GitPullRequest, ListNotesOptions, LongFormNoteOptions, MemoryEntry, Message,
    MintedInvite, ModerationReport, Note, NoteAuthor, PresenceInfo, PresenceStatus, ProfileField,
    Reaction, ReadState, RelayAgentInfo, RelayInformation, RelayMember, Reminder, ReminderTarget,
    RepoProject, ThreadSummary, TuiMessageView, TuiRelayClient, UserProfile, UserStatus,
    WindowCursor, Workflow, WorkflowDetail, WorkflowRun, CHANNEL_WINDOW_HEAD_LIMIT,
    CHANNEL_WINDOW_PAGE_LIMIT,
};
use crate::identity::IdentityStore;
use crate::live::LiveChannelTarget;
use crate::memory::selected_agent_memory_identity;
use crate::refresh::{
    ChannelWindowMeta, HydrateResult, HydrateTarget, RefreshResult, RefreshTarget, SidebarData,
};
...
pub struct App {
    ...
    pub channel_sort_modes: BTreeMap<String, String>,
    ...
    /// Set when a cached channel was painted and the main loop should request
...
impl App {
    pub fn new(config: AppConfig) -> Self {
        ...
        Self {
            ...
            channel_sort_modes: BTreeMap::new(),
    ...
    async fn load_sidebar_channels(&self) -> Result<(Vec<Channel>, Option<String>), String> {
        let (mut channels, warning) = match self.channel_scope {
        ...
        if let Ok(client) = self.native_relay_client() {
            client.hydrate_channel_last_message_at(&mut channels).await;
        }
        Ok((channels, warning))
    }
    ...
    pub fn apply_refresh_result(&mut self, target: &RefreshTarget, result: RefreshResult) {
        ...
        if let Some(modes) = result.channel_sort_modes {
            self.channel_sort_modes = modes;
        }
        self.sort_sidebar_channels();
    ...
    async fn mark_visible_thread_messages_read(&mut self) {
        ...
        self.publish_workspace_read_state().await;
    }
    ...
    async fn sync_channel_sort(&mut self) {
        let Ok(client) = self.native_relay_client() else {
            return;
        };
        if let Ok(store) = client.channel_sort_store().await {
    ...
    async fn publish_workspace_read_state(&mut self) -> bool {
        let contexts = self
            .workspace_config
            .read_frontiers
        ...
        match self.publish_read_state_native(contexts).await {
            Ok(()) => true,
            Err(error) => {
    ...
    async fn publish_read_state_native(
        ...
        let client_id = self.workspace_config.read_state_client_id.clone();
        let slot_id = self.workspace_config.read_state_slot_id.clone();
        let client = self.native_relay_client()?;
        let remote_created_at = client
            .read_state_slot_created_at(&slot_id)
            .await
            .map_err(|error| error.to_string())?;
        let created_at = now_seconds().max(remote_created_at.saturating_add(1));
        let event = client
            .build_read_state_event(&client_id, &slot_id, contexts, Some(created_at))
            .map_err(|error| error.to_string())?;
        client
            .submit_event(&event)
            .await
    ...
    pub(super) async fn send_message_native(
        ...
        client
            .send_channel_message(channel_id, surface, content, reply_to)
            .await
            .map_err(|error| error.to_string())
    }
    ...
    async fn list_relay_agents_native(&self) -> Result<Vec<RelayAgentInfo>, String> {
        let client = self.native_relay_client()?;
        client
            .relay_agents()
            .await
    ...
    async fn send_message_with_files_native(
        ...
        let surface = self.channel_message_surface(channel_id);
        client
            .send_channel_message_with_files(channel_id, surface, content, reply_to, files)
            .await
            .map(|_| ())
            .map_err(|error| error.to_string())
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn requested_relay_directory_refresh_is_consumed_once() {
        let mut app = test_app();
        app.set_agent_panel_visible(true);

        app.refresh_requested_relay_agents().await;

        assert!(!app.relay_agents_refresh_requested);
        assert_eq!(app.status, "relay agents: BUZZ_PRIVATE_KEY is required");
    }
    ...
    #[test]
    fn insert_selected_contact_mention_adds_nip27_uri_to_composer() {
        ...
        assert_eq!(
            app.composer,
            "@Ada Lovelace nostr:npub10elfcs4fr0l0r8af98jlmgdh9c8tcxjvz9qkw038js35mp4dma8qzvjptg "
        );
        assert!(app.has_channel_draft("one"));
    }

    #[test]
    fn agent_entries_exclude_local_relay_matches_from_directory() {
    ...
    #[test]
    fn agent_display_entries_separate_local_and_relay_directory_sections() {
        const LOCAL: &str = "7e7e9c42a91bfef19fa929e5fda1b72e0ebc1a4c1141673e2794234d86addf4e";
        const RELAY: &str = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let mut app = test_app();
        app.acp
            .upsert_runtime(test_managed_runtime(LOCAL, "Local"), None, None);
        app.relay_agents = vec![test_relay_agent(RELAY, "Relay Only")];
        ...
        assert!(matches!(
            ...
            [
                ...
                AgentDisplayEntry::Header("Relay directory"),
                AgentDisplayEntry::Entry(AgentListEntry::Relay(_)),
            ]
        ));
    ...
    #[test]
    fn insert_selected_relay_agent_mention_adds_nip27_uri_to_composer() {
        const AGENT: &str = "7e7e9c42a91bfef19fa929e5fda1b72e0ebc1a4c1141673e2794234d86addf4e";
        let mut app = test_app();
        app.channels = vec![test_channel("one", "one")];
        ...
        assert_eq!(
            app.composer,
            "@Relay Helper nostr:npub10elfcs4fr0l0r8af98jlmgdh9c8tcxjvz9qkw038js35mp4dma8qzvjptg "
        );
    }
    ...
    fn mention_completion_includes_reachable_relay_agents() {
        ...
        assert!(app.accept_completion());
        let uri = nostr_pubkey_uri(pubkey).expect("valid pubkey");
        assert_eq!(app.composer, format!("ask @Relay Helper {uri} "));
    }
    ...
    fn mention_completion_preserves_same_name_agents_by_pubkey() {
        ...
        let completion = app.composer_completion.as_ref().expect("completion");
        let inserts = completion
            .matches
            .iter()
            .map(|item| item.insert.as_str())
            .collect::<Vec<_>>();
        let local_uri = nostr_pubkey_uri(local_pubkey).expect("valid local pubkey");
        let relay_uri = nostr_pubkey_uri(relay_pubkey).expect("valid relay pubkey");
        assert_eq!(inserts.len(), 2);
        assert!(inserts.iter().any(|insert| insert.contains(&local_uri)));
        assert!(inserts.iter().any(|insert| insert.contains(&relay_uri)));
    ...
    #[test]
    fn followed_thread_reply_alert_ignores_active_thread_and_own_messages() {
        let mut app = test_app();
