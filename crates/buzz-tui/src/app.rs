...
use crate::cli::{
    BuzzCli, Channel, ChannelMember, ChannelPreferenceKind, ChannelSection, ChannelSections,
    Contact, ConversationKind, CreateChannelOptions, CreateIssueOptions, CreatePatchOptions,
    CreateRepoOptions, CustomEmojiEntry, EmojiExportScope, GitIssue, GitPatch, ListNotesOptions,
    LongFormNoteOptions, MemoryEntry, Message, Note, NoteAuthor, PresenceInfo, PresenceStatus,
    ProfileField, Reaction, ReadState, RepoProject, UserProfile, Workflow, WorkflowDetail,
    WorkflowRun,
};
...
use buzz_tui_client::app_data::{ChannelPreferenceEntry, ChannelPreferenceStoreKind};
use buzz_tui_client::{TuiMessageView, TuiRelayClient};
...
pub use composer::{CompletionItem, CompletionKind, CompletionState};
pub use confirm::{ConfirmAction, ConfirmState};
pub use hints::Hint;
pub use navigation::NavigationEntry;
pub use palette::{Command, CommandScope};
...
impl App {
    pub fn new(
        ...
        acp_binary: String,
        startup_notice: Option<String>,
        managed_agent_store: ManagedAgentStore,
        managed_agent_store_path: PathBuf,
                ...
                if let Some(channel) = self.active_channel() {
                    ...
                    match messages {
                        Ok(messages) => {
                            ...
                            } else if self.restore_selected_channel_message(selected_id.as_deref())
                            {
                                self.reset_message_detail_scroll();
                            }
    ...
    pub async fn apply_live_message(&mut self, message: TuiMessageView) -> bool {
    ...
    pub async fn move_selection(&mut self, delta: isize) {
        match self.focus {
            ...
            Focus::Timeline => match self.timeline_mode {
                ...
                TimelineMode::Search => {
                    let before = self.selected_search_result;
                    move_index(
                        &mut self.selected_search_result,
                        self.search_results.len(),
                        delta,
                    );
                    if self.selected_search_result != before {
                        self.reset_message_detail_scroll();
                    }
                    self.refresh_selected_message_reactions().await;
                }
                TimelineMode::Feed => {
                    let before = self.selected_feed;
                    move_index(&mut self.selected_feed, self.feed.len(), delta);
                    if self.selected_feed != before {
                        self.reset_message_detail_scroll();
                    }
                    self.refresh_selected_message_reactions().await;
                }
                TimelineMode::Pulse => {
                    let before = self.selected_pulse;
                    move_index(&mut self.selected_pulse, self.pulse.len(), delta);
                    if self.selected_pulse != before {
                        self.reset_message_detail_scroll();
                    }
                    self.refresh_selected_message_reactions().await;
                }
    ...
    pub async fn export_workspace_emoji(&mut self) {
        ...
        match self
            .export_emoji_json_native(EmojiExportScope::Workspace)
            .await
        {
    ...
    pub fn next_create_channel_field(&mut self) {
        self.new_channel_field = match self.new_channel_field {
            ...
            CreateChannelField::Visibility => CreateChannelField::Description,
    ...
    pub fn previous_create_channel_field(&mut self) {
        self.new_channel_field = match self.new_channel_field {
            ...
            CreateChannelField::Description => CreateChannelField::Visibility,
        };
    }
    ...
    async fn publish_channel_read_at(&mut self, channel_id: &str, latest: u64) -> bool {
        ...
        match self
            .publish_channel_read_at_native(channel_id, latest)
            .await
        {
            Ok(()) => true,
            Err(error) => {
    ...
    async fn publish_channel_read_at_native(
        &self,
        channel_id: &str,
        latest: u64,
    ) -> Result<(), String> {
        ...
        let event = client
            .build_read_state_event(
                BTreeMap::from([(channel_id.to_string(), latest)]),
                Some(latest),
            )
    ...
    async fn get_canvas_native(
        &self,
        channel_id: &str,
    ) -> Result<crate::cli::CanvasDocument, String> {
        let client = self.native_relay_client()?;
        client
    ...
    async fn export_emoji_json_native(&self, scope: EmojiExportScope) -> Result<String, String> {
        let client = self.native_relay_client()?;
        client
            .export_emoji_json(scope)
            .await
            .map_err(|error| error.to_string())
    }
    ...
    async fn send_diff_native(
        &self,
        channel_id: &str,
        options: &crate::cli::SendDiffOptions,
        reply_to: Option<&str>,
    ) -> Result<(), String> {
        let client = self.native_relay_client()?;
        client
    ...
    async fn upload_file_native(&self, path: &str) -> Result<crate::cli::UploadedFile, String> {
        let client = self.native_relay_client()?;
        client
            .upload_file(path)
            .await
    ...
    fn native_relay_client_with_identity(
        ...
        if private_key.is_empty() {
            return Err("BUZZ_PRIVATE_KEY is required".to_string());
        }
    ...
    fn selected_new_channel_input_mut(&mut self) -> Option<&mut String> {
        match self.new_channel_field {
            ...
            CreateChannelField::Type | CreateChannelField::Visibility => None,
        }
    }
...
mod tests {
    ...
    fn test_app() -> App {
        ...
        App::new(
            BuzzCli::new("http://localhost:3000".to_string(), None, None),
            ...
            PathBuf::from("/tmp/buzz-tui-test-agents.json"),
            workspace_config,
            PathBuf::from("/tmp/buzz-tui-test-workspaces.json"),
        )
    }
    ...
    #[test]
    fn create_channel_form_cycles_fields() {
        let mut app = test_app();

        assert_eq!(app.new_channel_field, CreateChannelField::Name);
        app.next_create_channel_field();
        assert_eq!(app.new_channel_field, CreateChannelField::Type);
        app.next_create_channel_field();
        assert_eq!(app.new_channel_field, CreateChannelField::Visibility);
        app.next_create_channel_field();
        assert_eq!(app.new_channel_field, CreateChannelField::Description);
        app.next_create_channel_field();
        assert_eq!(app.new_channel_field, CreateChannelField::Name);
    ...
    #[test]
    fn create_channel_form_routes_text_to_selected_field() {
        let mut app = test_app();

        app.new_channel_push('r');
        app.new_channel_push('d');
        app.next_create_channel_field();
        app.new_channel_push('x');
        app.next_create_channel_field();
        app.new_channel_push('y');
        app.next_create_channel_field();
        app.new_channel_push('R');
        app.new_channel_push('D');
        app.new_channel_pop();

        assert_eq!(app.new_channel_name, "rd");
        assert_eq!(app.new_channel_type, NewChannelType::Stream);
        assert_eq!(app.new_channel_visibility, NewChannelVisibility::Open);
        assert_eq!(app.new_channel_description, "R");
    }

    #[test]
    fn create_channel_form_cycles_type_and_visibility() {
        let mut app = test_app();

        app.cycle_new_channel_type();
        app.cycle_new_channel_visibility();
        assert_eq!(app.new_channel_type, NewChannelType::Forum);
        assert_eq!(app.new_channel_visibility, NewChannelVisibility::Private);
    ...
    #[test]
    fn workflow_create_editor_uses_active_channel() {
        let mut app = test_app();
        app.channels = vec![test_channel("channel-1", "engineering")];

        app.focus_create_workflow();
        app.workflow_yaml_push('n');
        app.workflow_yaml_push('a');
        app.workflow_yaml_pop();

        assert_eq!(app.focus, Focus::WorkflowEdit);
        assert!(!app.workflow_edit_existing);
        assert_eq!(app.workflow_channel_id, "channel-1");
        assert_eq!(app.workflow_yaml, "n");
    }

    #[test]
    fn workflow_edit_editor_seeds_selected_workflow() {
        let mut app = test_app();
        app.workflow_channel_id = "channel-1".to_string();
        app.workflows = vec![Workflow {
            workflow_id: "workflow-1".to_string(),
            content: "name: build".to_string(),
            ..Workflow::default()
        }];
        ...
        assert_eq!(app.focus, Focus::WorkflowEdit);
        assert!(app.workflow_edit_existing);
        assert_eq!(app.workflow_edit_id.as_deref(), Some("workflow-1"));
        assert_eq!(app.workflow_yaml, "name: build");
    }

    #[test]
    fn workflow_inputs_editor_seeds_selected_workflow() {
        let mut app = test_app();
        app.workflows = vec![Workflow {
            workflow_id: "workflow-1".to_string(),
            content: "name: build".to_string(),
        ...
        app.workflow_inputs_pop();
        app.workflow_inputs_push('}');

        assert_eq!(app.focus, Focus::WorkflowInputs);
        assert_eq!(app.workflow_edit_id.as_deref(), Some("workflow-1"));
        assert_eq!(app.workflow_inputs, "{}");
    }

    #[test]
    fn workflow_approval_editor_routes_token_and_note() {
        let mut app = test_app();

        app.focus_workflow_approval(false);
        app.workflow_approval_push('t');
        app.workflow_approval_push('1');
        app.next_workflow_approval_field();
        app.workflow_approval_push('n');
        app.workflow_approval_push('o');
        app.workflow_approval_pop();

        assert_eq!(app.focus, Focus::WorkflowApproval);
        assert!(!app.workflow_approval_approved);
        assert_eq!(app.workflow_approval_token, "t1");
        assert_eq!(app.workflow_approval_note, "n");
        assert_eq!(app.workflow_approval_field, WorkflowApprovalField::Note);
        app.previous_workflow_approval_field();
        assert_eq!(app.workflow_approval_field, WorkflowApprovalField::Token);
    }

    #[test]
    fn memory_create_editor_routes_slug_and_value() {
        let mut app = test_app();
        app.memory_agent_pubkey = "agent-1".to_string();

        app.focus_create_memory();
        app.memory_input_push('m');
        app.memory_input_push('1');
        app.next_memory_edit_field();
        app.memory_input_push('v');
        app.memory_input_push('x');
        app.memory_input_pop();

        assert_eq!(app.focus, Focus::MemoryEdit);
        assert!(!app.memory_edit_existing);
        assert_eq!(app.memory_slug, "m1");
        assert_eq!(app.memory_value, "v");
        assert_eq!(app.memory_edit_field, MemoryEditField::Value);
        app.previous_memory_edit_field();
        assert_eq!(app.memory_edit_field, MemoryEditField::Slug);
    }

    #[test]
    fn memory_edit_editor_seeds_selected_memory() {
        let mut app = test_app();
        app.memories = vec![MemoryEntry {
            slug: "mem/values/honesty".to_string(),
            value: "tell the truth".to_string(),
            ..MemoryEntry::default()
        }];
        ...
        assert_eq!(app.focus, Focus::MemoryEdit);
        assert!(app.memory_edit_existing);
        assert_eq!(app.memory_slug, "mem/values/honesty");
        assert_eq!(app.memory_value, "tell the truth");
        assert_eq!(app.memory_edit_field, MemoryEditField::Value);
    }

    #[test]
    fn emoji_import_editor_tracks_path_and_replace_mode() {
        let mut app = test_app();

        app.focus_import_emoji();
        app.emoji_import_push('/');
        app.emoji_import_push('t');
        app.emoji_import_push('x');
        app.emoji_import_pop();
        app.toggle_emoji_import_replace();

        assert_eq!(app.focus, Focus::EmojiImport);
        assert_eq!(app.emoji_import_path, "/t");
        assert!(app.emoji_import_replace);
    }
    ...
    fn palette_ranks_contextual_commands_and_explains_disabled_actions() {
        ...
        assert_eq!(
            commands.first().map(|command| command.scope),
            Some(CommandScope::Timeline)
        );
        let delete = commands
            .iter()
    ...
    async fn live_message_merges_active_channel_timeline() {
        ...
        assert!(
            app.apply_live_message(TuiMessageView {
                id: "newer".to_string(),
    ...
    #[test]
    fn channel_section_assignment_prompt_prefills_current_section() {
        let mut app = test_app();
        app.channels = vec![test_channel("channel-1", "engineering")];
        app.channel_sections = vec![ChannelSection {
            id: "core".to_string(),
        ...
        assert_eq!(app.focus, Focus::ChannelSectionAssign);
        assert_eq!(app.channel_action_input, "Core Work");
    }
    ...
    async fn live_relay_smoke_exercises_tui_paths() {
        ...
        let acp = AcpSupervisor::new(AcpSupervisorConfig {
            ...
            relay_url: buzz_tui_client::relay_http_to_ws_url(&relay),
        ...
        let mut app = App::new(
            cli.clone(),
            acp,
            acp_script.display().to_string(),
            None,
            ManagedAgentStore::default(),
            temp_dir.join("agents.json"),
            WorkspaceConfig::with_default(&relay),
            workspace_store,
        );
