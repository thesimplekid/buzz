...
use buzz_core::kind::{
    ...
    KIND_DM_HIDE, KIND_DM_OPEN, KIND_DM_VISIBILITY, KIND_EMOJI_SET, KIND_EVENT_REMINDER,
    ...
    KIND_GIT_STATUS_MERGED, KIND_GIT_STATUS_OPEN, KIND_HTTP_AUTH, KIND_LONG_FORM,
    KIND_MANAGED_AGENT, KIND_NIP29_GROUP_MEMBERS, KIND_NIP29_GROUP_METADATA,
    KIND_NIP43_MEMBERSHIP_LIST, KIND_PRESENCE_SNAPSHOT, KIND_PRESENCE_UPDATE, KIND_REACTION,
    KIND_READ_STATE, KIND_REPORT, KIND_STREAM_MESSAGE, KIND_STREAM_MESSAGE_EDIT,
    KIND_STREAM_MESSAGE_V2, KIND_TEXT_NOTE, KIND_TYPING_INDICATOR, KIND_USER_STATUS,
...
use crate::client::app_data::{
    ...
    channel_sections_payload, channel_sort_tags, read_state_payload, read_state_tags,
    ChannelPreferenceEntry, ChannelPreferenceStoreKind, ChannelSectionRecord, ChannelSectionStore,
    ChannelSortStore, ReadStateBlob, CHANNEL_SECTIONS_D_TAG, CHANNEL_SORT_D_TAG,
};
...
impl From<reqwest::Error> for RelayClientError {
    fn from(error: reqwest::Error) -> Self {
        RelayClientError::Http(format_reqwest_error(&error))
    }
}
...
impl TuiRelayClient {
    ...
    pub fn build_channel_message_event(
        ...
        let reply = reply_to
            ...
            .transpose()?;
        self.build_message_event(uuid, surface, content, reply, &[])
    }
    ...
    pub async fn send_channel_message(
        ...
        let event = self.build_message_event(uuid, surface, content, reply, &[])?;
    ...
    pub async fn send_channel_message_with_files(
        ...
        let reply = self.resolve_reply_pair(reply_to).await?;
        let event = self.build_message_event(uuid, surface, &final_content, reply, &media_tags)?;
    ...
    pub async fn relay_agents(&self) -> Result<Vec<RelayAgentInfo>, RelayClientError> {
        ...
        Ok(latest.into_values().map(|(_, agent)| agent).collect())
    }
    ...
    pub async fn moderation_reports(&self) -> Result<Vec<ModerationReport>, RelayClientError> {
        let response = self
            .bridge_get("/moderation/reports?status=open&limit=100")
            .await?;
    ...
    pub async fn mint_invite(
        ...
        let body = serde_json::to_vec(
            &ttl_secs.map_or_else(|| json!({}), |ttl_secs| json!({ "ttl_secs": ttl_secs })),
        )?;
        let response = self.bridge_post("/api/invites", body).await?;
        Ok(response.json().await?)
    }
    ...
    /// The synced per-group channel sort preference (desktop `channel-sort`
    /// NIP-78 blob). Returns an empty store when none is published.
    pub async fn channel_sort_store(&self) -> Result<ChannelSortStore, RelayClientError> {
        Ok(self
            .fetch_channel_sort_blob()
            .await?
            .map(|(store, _)| store)
            .unwrap_or(ChannelSortStore {
                version: 1,
                groups: BTreeMap::new(),
            }))
    }

    async fn fetch_channel_sort_blob(
        &self,
    ) -> Result<Option<(ChannelSortStore, u64)>, RelayClientError> {
        let value = self
            .query_values(&[json!({
                "kinds": [KIND_READ_STATE],
                "authors": [self.public_key_hex()],
                "#d": [CHANNEL_SORT_D_TAG],
                "limit": 1,
            })])
            .await?;
        let Some(event) = value.as_array().into_iter().flatten().next() else {
        ...
        // Desktop encrypts the blob to self with NIP-44; tolerate plaintext.
        let plaintext = nip44::decrypt(self.keys.secret_key(), &self.keys.public_key(), content)
            .unwrap_or_else(|_| content.to_string());
        let Ok(store) = serde_json::from_str::<ChannelSortStore>(&plaintext) else {
            return Ok(None);
        };
        Ok(store.sanitized().map(|store| (store, created_at)))
    }

    /// Read-modify-write one group's sort mode into the shared blob. The blob
    /// is whole-blob last-write-wins, so unrelated groups are preserved and
    /// `created_at` is bumped past the remote head.
    pub async fn set_channel_sort_mode(
        &self,
        group_key: &str,
        mode: &str,
    ) -> Result<ChannelSortStore, RelayClientError> {
        let (mut store, remote_created_at) = self.fetch_channel_sort_blob().await?.unwrap_or((
            ChannelSortStore {
                version: 1,
                groups: BTreeMap::new(),
            },
        ...
        store.version = 1;
        store.groups.insert(group_key.to_string(), mode.to_string());
        ...
        .map_err(|error| RelayClientError::Signing(error.to_string()))?;
        let tags =
            channel_sort_tags().map_err(|error| RelayClientError::Builder(error.to_string()))?;
        let created_at = monotonic_reminder_created_at(remote_created_at);
        let event = self.sign_event(
            EventBuilder::new(Kind::Custom(KIND_READ_STATE as u16), ciphertext)
                .tags(tags)
                .custom_created_at(Timestamp::from(created_at)),
        )?;
        self.submit_event(&event).await?;
    ...
    /// Fill `last_message_at` on each channel from the newest chat message.
    /// Uses per-channel single-`#h` filters so the relay can serve each from
    /// its channel index; a multi-value `#h` filter is not index-pushed and
    /// silently drops quieter channels under the global limit.
    pub async fn hydrate_channel_last_message_at(&self, channels: &mut [Channel]) {
        ...
        let filters: Vec<Value> = channels
            ...
            .collect();
        let Ok(value) = self.query_values(&filters).await else {
            return;
        };
        ...
        for event in value.as_array().into_iter().flatten() {
            ...
            let created_at = event
                ...
                .and_then(Value::as_u64)
                .unwrap_or_default();
            let entry = latest.entry(channel_id).or_insert(0);
            if created_at > *entry {
                *entry = created_at;
            }
        }
        for channel in channels {
            if let Some(created_at) = latest.get(&channel.id) {
                channel.last_message_at = *created_at;
            }
        }
    }
    ...
    pub async fn relay_information(&self) -> Result<RelayInformation, RelayClientError> {
        ...
        cache
            .lock()
            .expect("relay information cache poisoned")
            .insert(self.base_url.clone(), info.clone());
        Ok(info)
    }
    ...
    async fn bridge_post(
        ...
        if !response.status().is_success() {
            ...
            return Err(RelayClientError::Http(if detail.is_empty() {
                format!("HTTP {status}")
            } else {
    ...
    async fn bridge_get(&self, path: &str) -> Result<reqwest::Response, RelayClientError> {
        let url = self.bridge_url(path);
        let response = self
            .with_bridge_headers(self.http.get(&url), "GET", &url, None)?
            .send()
            .await?;
        if !response.status().is_success() {
            let status = response.status();
...
pub fn normalize_relay_http_url(relay_url: &str) -> String {
    ...
    let http_url = if let Some(rest) = relay_url.strip_prefix("ws://") {
        format!("http://{rest}")
    } else if let Some(rest) = relay_url.strip_prefix("wss://") {
        format!("https://{rest}")
    } else {
        relay_url.to_string()
    };
    normalize_loopback_http_url(&http_url)
}
...
fn normalize_loopback_http_url(relay_url: &str) -> String {
    let Ok(mut parsed) = url::Url::parse(relay_url) else {
...
fn parse_relay_agent_event(event: &Event) -> Option<RelayAgentInfo> {
    ...
    let owner_pubkey = (kind == KIND_MANAGED_AGENT).then(|| event.pubkey.to_hex());
    ...
    let respond_to = object
        ...
        .and_then(Value::as_str)
        .filter(|value| matches!(*value, "owner-only" | "allowlist" | "anyone"))
        .map(str::to_string);
...
mod tests {
    ...
    fn parse_relay_agent_event_preserves_public_respond_to_mode() {
        let event = agent_profile_event(
            r#"{"name":"Scout","respond_to":"anyone","respond_to_allowlist":["abcd"]}"#,
        );
        let agent = parse_relay_agent_event(&event).unwrap();

        assert_eq!(agent.respond_to.as_deref(), Some("anyone"));
        assert_eq!(agent.respond_to_allowlist, vec!["abcd"]);
    }
    ...
    #[test]
    fn normalize_relay_http_url_uses_ipv4_loopback_for_localhost() {
        assert_eq!(
            normalize_relay_http_url("http://localhost:3000"),
            "http://127.0.0.1:3000"
        );
        assert_eq!(
            normalize_relay_http_url("http://[::1]:3000"),
            "http://127.0.0.1:3000"
        );
        assert_eq!(
            normalize_relay_http_url("https://relay.example"),
            "https://relay.example"
        );
    }
    ...
    fn dm_metadata_is_classified_as_direct_message() {
        let channel = parse_channel_metadata_event(&json!({
            "pubkey": "owner",
            "created_at": 10,
            "tags": [["d", WINDOW_CHANNEL], ["name", "Alice"], ["t", "dm"], ["private"]]
        }))
        .unwrap();
