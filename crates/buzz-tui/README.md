...
- `C`: load your contact list. Use `Up` / `Down` to select a contact,
  ...
  composer as a NIP-27 `nostr:npub...` mention.
...
- `s`: when the sidebar is focused, toggle the selected channel's group between
  alphabetical and recent-activity sorting. The preference syncs with the
  desktop through the shared NIP-78 `channel-sort` blob, and the sidebar orders
  starred channels, sections, channels, forums, and DMs as groups.
