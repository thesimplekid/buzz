...
`buzz-tui` is a standalone Ratatui terminal client for Buzz. It uses
`buzz-client` for authenticated relay transport and supervises local ACP
harness processes for agents:

- Direct Buzz event operations for channels, messages, profile, notes, emoji,
  workflows, memory, and workspace state.
...
the relay event model and authenticated transport from one exact
[Buzz revision](Cargo.toml).
...
Then run it against a relay. `buzz-acp` must be on `PATH`, or supplied with
`--acp-bin`:

```bash
just run --relay http://localhost:3000
```

On first launch, the terminal setup flow asks for the relay and lets you either
generate a new identity or import an existing `nsec`/hex key. Generated keys
must be backed up before setup continues. Buzz stores the identity in the OS
keyring when available and falls back to a user-only `tui-identity.key` file.
`BUZZ_PRIVATE_KEY` remains supported and overrides the persisted identity for
...
cargo run --locked -- \
  --acp-bin /path/to/buzz-acp \
  --mcp-command /path/to/buzz-dev-mcp
```

## Releases and compatibility

All Buzz crates in `Cargo.toml` are pinned to the same full Git commit ID.
Release archives are built by `scripts/package-release.sh` and contain
`buzz-tui`, `buzz-acp`, and `buzz-dev-mcp`; both sidecars are built from that
same revision. The external `--acp-bin` and `--mcp-command` options remain
supported for operators that supply wrappers or separately managed binaries.

Source releases and issue tracking live at
[block/buzz-tui](https://github.com/block/buzz-tui). See
...
- `--identity-file`: fallback identity path when the OS keyring is unavailable.
  Defaults to `$BUZZ_TUI_IDENTITY_FILE`, then the Buzz directory under
  `$XDG_CONFIG_HOME` or `~/.config`.

`--acp-bin` may also point at an externally supplied ACP wrapper. The TUI passes
the normal managed-agent arguments and environment through to that command;
process containment and host-path policy remain the wrapper operator's
responsibility.

## Controls

- `Ctrl-P` / `:`: open the command palette. This is the primary way to discover
  and run actions; type familiar phrases such as `create channel`, `show
  details`, `backup key`, or `switch workspace`.
- Run `Pair mobile device` from the command palette to transfer the active
  workspace and identity to Buzz mobile over NIP-AB. Scan the terminal QR code
  or press `c` to copy its pairing code, then confirm the six-digit security
...
- The detail and agent panels start collapsed. Use the command palette to
  toggle either panel or reset the layout. Panel visibility and sizes persist
  across launches and workspaces; views that require a detail panel reveal it
  temporarily.
...
- `y`: copy the selected timeline/feed/Pulse message body to the terminal
  clipboard.
- `y`: while the sidebar is focused, copy the selected conversation's full
  channel ID to the terminal clipboard. The full ID is also shown in the
  channel detail panel, including for DMs.
- Click an underlined attachment/link label in the Message panel to copy its
  complete URL through OSC 52 to the local terminal clipboard, then paste it
  into a local browser. Hold `Shift` while dragging to select terminal text
  while mouse handling is enabled.
...
- `C`: load your contact list. Use `Up` / `Down` to select a contact,
  `Enter` to open a DM, or `@` to insert the selected contact into the channel
  composer as a visible `@name` mention with a separate event `p` tag.
...
- The sidebar keeps conversations in stable alphabetical order within starred,
  custom-section, channel, forum, and DM groups. New messages update unread
  state without moving the conversation.
...
- Composer text is remembered per workspace, channel, and thread, persists
  across restarts in the workspace store, and shows a `draft` marker in the
  sidebar. Open the drafts inbox from the command palette ("Open drafts"):
  `Enter` opens a draft in its channel, `S` sends it (confirmed), and `D`
  discards it (confirmed).
- Spoilers use the desktop's `||text||` syntax (and lone-`||` block delimiters).
  Timeline previews mask spoiler content; the Message detail panel reveals it
  with a `▒` marker.
- Channel timelines load through relay-assembled NIP-CW windows when the relay
  supports them: replies stay in threads, roots show a cyan `↳N` reply badge
  that updates live, and `PageUp` pages older history with relay-signed
  cursors. Relays without window support fall back to plain history queries.
...
- `A`: when the ACP runtime panel is focused on a runtime template, create a
  managed agent from it. Type a name, optionally set model, system prompt, and
  max output token fields with `Tab`, choose its response policy with `F3`,
  optionally enter allowlist pubkeys, toggle start-on-launch with `F2`, toggle
  threaded direct mentions with `F4`, cycle thinking effort with `F5`, and
  press `Enter`. Thinking effort and token limits are forwarded to harnesses
  through their normalized env vars (`BUZZ_AGENT_*`, `GOOSE_*`).
...
- `@`: when the ACP runtime panel is focused on a managed agent, insert that
  agent into the channel composer as a visible `@name` mention with a separate
  event `p` tag.
...
- NIP-AB mobile pairing with pairing-relay discovery, terminal QR/code display,
  SAS verification, and encrypted identity transfer;
...
- managed-agent composer mentions using visible `@name` text plus separate
  mention `p` tags, matching the desktop client;
...
Managed-agent records are stored in a TUI-owned local JSON file. New agents'
private keys are stored in the OS keyring when one is available (the
`system-keyring` cargo feature, enabled by default); the JSON record then only
holds a `@keyring` sentinel. Headless builds can disable the feature with
`--no-default-features` to keep keys in the file. On startup, the TUI starts
records marked `start_on_launch` directly through the local ACP supervisor.
When records exist, it launches those configured agents with their stored
relay URL, ACP command, runtime command/args, MCP command, private key,
...
Every spawned harness carries a `BUZZ_TUI_AGENT=<pid>` environment marker. On
Linux, startup sweeps `/proc` for harness process groups whose owning TUI
process no longer exists and reaps them, mirroring the desktop's stale-harness
cleanup. The agents panel also shows recent decrypted NIP-AM turn metrics
(kind 44200) — model, token counts, cost, and stop reason — for managed agents
owned by the active key.
