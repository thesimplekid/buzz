...
use crate::client::app_data::thread_context_key;
use crate::client::{Channel, Message, TuiMessageView};

const OLDER_HISTORY_PAGE_SIZE: u32 = 500;

impl App {
    pub(super) fn is_own_pubkey(&self, pubkey: &str) -> bool {
        self.profile
            .as_ref()
            .is_some_and(|profile| profile.pubkey.eq_ignore_ascii_case(pubkey))
            || self
                .cli
                .public_key_hex()
                .is_some_and(|own_pubkey| own_pubkey.eq_ignore_ascii_case(pubkey))
    }

    pub(super) fn signal_followed_thread_reply(&mut self, message: &TuiMessageView) {
        let Some(root_id) = message
            .thread_root_id
            .as_deref()
        ...
        if self.thread_root.as_deref() == Some(root_id) || !self.is_thread_followed(root_id) {
            return;
        }
        if self.is_own_pubkey(&message.pubkey) {
            return;
        }
        self.attention_notice = Some(format!(
            "New reply in followed thread {}",
            super::short_id(root_id)
        ));
    }

    pub fn is_thread_followed(&self, root_id: &str) -> bool {
        self.workspace_config
            .thread_follows
            .get(self.active_workspace_id())
            .is_some_and(|entries| entries.contains_key(root_id))
    }

    pub fn thread_has_unread(&self, channel_id: &str, root_id: &str) -> bool {
        let Some(last_reply_at) = self
            .thread_summaries
            .get(root_id)
            .and_then(|summary| summary.last_reply_at)
        else {
            return false;
        };
        let frontiers = self
        ...
        let channel_frontier = frontiers
            .and_then(|contexts| contexts.get(channel_id))
            .copied()
            .unwrap_or_default();
        let thread_frontier = frontiers
            .and_then(|contexts| contexts.get(&thread_context_key(root_id)))
            .copied()
            .unwrap_or_default();
        last_reply_at > channel_frontier.max(thread_frontier)
    }

    pub fn toggle_selected_thread_follow(&mut self) {
        let root_id = self
            .thread_root
            .clone()
            .or_else(|| self.selected_timeline_message().map(|message| message.id));
        let Some(root_id) = root_id.filter(|id| !id.is_empty()) else {
            self.status = "No thread selected".to_string();
            return;
        };
        ...
        let entries = self
            ...
            .entry(workspace_id)
            .or_default();
        let followed = if entries.remove(&root_id).is_some() {
            false
        } else {
            entries.insert(root_id.clone(), super::now_seconds());
            while entries.len() > 500 {
                let Some(oldest) = entries
                    .iter()
                    .min_by_key(|(_, followed_at)| **followed_at)
                    .map(|(id, _)| id.clone())
                else {
                    break;
                };
        ...
        self.save_workspace_config("thread follows");
        self.status = format!(
            "{} thread {}",
            if followed { "Following" } else { "Unfollowed" },
            super::short_id(&root_id)
        );
    }

    /// Load one older page for the active channel/thread, preserving the
    /// selected message where possible.
    pub async fn load_older_messages(&mut self) {
        ...
        let until = older_history_until(&self.messages);
        let selected_id = self
            .selected_timeline_message()
            .map(|message| message.id.clone());
        let result = if let Some(root) = self.thread_root.clone() {
            self.get_thread_messages_before(&channel.id, &root, until, OLDER_HISTORY_PAGE_SIZE)
                .await
        } else if self.window_active {
            // NIP-CW: page with the relay-provided composite cursor; the
            // kind-39006 bounds are the only authority on exhaustion.
            let Some(cursor) = self.window_next_cursor.clone() else {
                self.status = "No older messages".to_string();
                return;
            };
            self.load_channel_window_older(&channel.id, &cursor).await
        } else {
            self.get_channel_messages_before(&channel.id, until, OLDER_HISTORY_PAGE_SIZE)
                .await
        };
        match result {
            Ok(page) => {
                self.remember_message_author_profiles(&page).await;
                let (messages, added) = merge_history_page(self.messages.clone(), page);
    ...
    pub(super) async fn load_selected_channel(&mut self) {
        ...
        self.begin_unread_session(&channel.id, None);
        ...
        self.loading_channel_id = Some(channel.id.clone());
        self.pending_active_refresh = true;

        // Paint the last-known timeline immediately. The event loop consumes
        // `pending_active_refresh` after this handler returns, so no relay work
        // delays the next terminal draw.
        if let Some(snapshot) = self.channel_snapshot_cache.get(&channel.id).cloned() {
            self.messages = snapshot.messages;
            self.thread_summaries = snapshot.summaries;
            self.window_active = snapshot.window_active;
            self.window_has_more = snapshot.window_has_more;
            self.window_next_cursor = snapshot.window_next_cursor;
            ...
            self.status = format!("Opened #{}; refreshing…", channel.name);
            return;
        }
        ...
        self.reset_message_detail_scroll();
        self.restore_active_channel_draft();
        self.status = format!("Opening #{}…", channel.name);
    }
    ...
    pub(super) async fn open_context_channel(&mut self, channel: Channel) {
        self.begin_unread_session(&channel.id, None);
        self.thread_root = None;
        ...
        self.pulse_reply_target = None;
        self.selected_channel_detail = Some(channel.clone());
        match self.load_channel_window_head(&channel.id).await {
            Ok(messages) => {
    ...
    pub(super) async fn open_selected_thread(&mut self) {
        ...
        match self.get_thread_messages(&channel.id, thread_root).await {
            Ok(messages) => {
                self.begin_unread_session(&channel.id, Some(thread_root));
                self.remember_message_author_profiles(&messages).await;
    ...
    pub(super) async fn open_selected_feed_thread(&mut self) {
            ...
            .await
        {
            Ok(messages) => {
                ...
                self.remember_message_author_profiles(&messages).await;
                self.begin_unread_session(&message.channel_id, Some(&message.id));
    ...
    pub(super) async fn open_selected_search_thread(&mut self) {
    ...
    pub fn copy_selected_message_to_clipboard(&mut self) {
        let Some(message) = self.selected_timeline_message() else {
            self.status = "No message selected".to_string();
            return;
        };
        if message.content.is_empty() {
            self.status = "Selected message is empty".to_string();
            return;
        }
        match crate::clipboard::copy_text(&message.content) {
            Ok(()) => {
                self.status = format!("Copied message {} to clipboard", short_id(&message.id));
            }
            Err(error) => {
                self.status = format!("copy message: {error}");
            }
        }
    }
    ...
    pub fn scroll_message_detail_down(&mut self) {
        ...
        let opened = !self.detail_panel_visible;
        if opened {
            self.set_detail_panel_visible(true);
        }
        self.message_detail_scroll = self.message_detail_scroll.saturating_add(8);
        self.status = if opened {
            "Opened and scrolled message detail".to_string()
        } else {
    ...
    pub fn scroll_message_detail_bottom(&mut self) {
        ...
        self.status = if opened {
            "Opened message detail at bottom".to_string()
        } else {
            "Message detail at bottom".to_string()
        };
    }
    ...
    pub async fn refresh_selected_message_reactions(&mut self) {
        ...
        match self.get_message_reactions(&message.id).await {
            Ok(reactions) => {
                self.selected_reactions = reactions;
                let actors = self.unknown_reaction_actor_pubkeys();
                self.remember_profiles_for_pubkeys(actors).await;
            }
            Err(error) => {
                self.selected_reactions.clear();
...
pub(super) fn older_history_until(messages: &[Message]) -> Option<u64> {
    messages
        .iter()
        .filter(|message| message.created_at > 0)
        .map(|message| message.created_at)
        .min()
        .map(|created_at| created_at.saturating_sub(1))
}

pub(super) fn merge_history_page(
    ...
    for message in page.into_iter().chain(existing) {
        if message.id.is_empty() {
            idless.push(message);
        } else {
    ...
    let mut merged = by_id.into_values().chain(idless).collect::<Vec<_>>();
    merged.sort_by_key(|message| (message.created_at, message.id.clone()));
...
mod tests {
    ...
    #[test]
    fn older_history_until_uses_before_oldest_loaded_message() {
        assert_eq!(
            older_history_until(&[message("newer", 20), message("older", 10)]),
            Some(9)
        );
    }

    #[test]
    fn merge_history_page_orders_and_dedupes_by_event_id() {
        let existing = vec![message("b", 20), message("c", 30)];
        let page = vec![message("a", 10), message("b", 20)];

        let (merged, added) = merge_history_page(existing, page);
