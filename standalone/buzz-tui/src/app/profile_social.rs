use super::{
    clamp_index, is_hex64, nostr_pubkey_uri, parse_contact_input, presence_status_from_info,
    profile_field_label, profile_label, short_id, App, Focus, ProfileField, PulseSource,
    TimelineMode,
};
use crate::client::{Message, PresenceStatus, UserProfile};
use std::cmp::Reverse;
...
impl App {
    ...
    pub async fn focus_profile(&mut self) {
        match self.current_profile().await {
            Ok(profile) => {
                ...
                self.user_status = match self.profile.as_ref().map(|profile| profile.pubkey.clone())
                {
                    Some(pubkey) => self.user_status(&pubkey).await.unwrap_or_default(),
                    None => None,
                };
    ...
    pub fn insert_selected_contact_mention(&mut self) {
        let Some(contact) = self.contacts.get(self.selected_contact).cloned() else {
            self.status = "Select a contact to mention".to_string();
            return;
        };
        if self.timeline_mode == TimelineMode::Pulse {
            self.status = "Contact mentions are for channel messages".to_string();
            return;
        }
        ...
        let label = if contact.petname.trim().is_empty() {
            ...
            contact.petname.trim().to_string()
        };
        if nostr_pubkey_uri(&contact.pubkey).is_none() {
            self.status = format!(
                "contact {} has an invalid pubkey",
                short_id(&contact.pubkey)
            );
            return;
        }
        self.append_composer_mention(&label, &contact.pubkey);
        self.focus = Focus::Composer;
        self.status = format!("Mentioned contact {label}");
    }
    ...
    pub(super) fn edit_profile_field(&mut self) {
        self.profile_input = if self.selected_profile_field == ProfileField::Status {
            self.user_status
                .as_ref()
                .map(|status| {
            ...
            self.profile_field_value().to_string()
        };
        self.focus = Focus::ProfileEdit;
        self.status = if self.selected_profile_field == ProfileField::Status {
            "Editing custom status as 'emoji | text'; empty clears it".to_string()
        } else {
            format!(
                "Editing {}",
    ...
    pub(super) async fn save_profile_field(&mut self) {
        ...
        if self.selected_profile_field == ProfileField::Status {
            let (emoji, text) = parse_user_status_input(&value);
            match self.set_user_status(&text, &emoji).await {
                Ok(()) => {
                    self.user_status = (!text.is_empty() || !emoji.is_empty()).then(|| {
                        crate::client::UserStatus {
                            text,
                            emoji,
                            updated_at: super::now_seconds(),
                        }
                    });
                    self.profile_input.clear();
                    self.focus = Focus::Profile;
                    self.status = if self.user_status.is_some() {
                        "Saved custom status".to_string()
                    } else {
                        "Cleared custom status".to_string()
                    };
                }
                Err(error) => self.status = format!("custom status: {error}"),
            }
            return;
        }
        if value.is_empty() {
...
fn parse_user_status_input(input: &str) -> (String, String) {
    match input.split_once('|') {
        Some((emoji, text)) => (emoji.trim().to_string(), text.trim().to_string()),
...
mod tests {
    ...
    #[test]
    fn parses_status_with_optional_emoji_prefix() {
        assert_eq!(
            parse_user_status_input("🚧 | Heads down"),
            ("🚧".to_string(), "Heads down".to_string())
        );
        assert_eq!(
            parse_user_status_input("Available"),
            (String::new(), "Available".to_string())
        );
        assert_eq!(
            parse_user_status_input(" | "),
