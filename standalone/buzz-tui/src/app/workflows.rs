...
impl App {
    ...
    pub async fn save_workflow(&mut self) {
        ...
        match result {
            Ok((response, saved_id)) => {
                let webhook_secret = response
                    .get("webhook_secret")
                    .and_then(serde_json::Value::as_str)
                    .map(str::to_string);
                ...
                self.status = match webhook_secret {
                    Some(secret) => match crate::clipboard::copy_text(&secret) {
                        Ok(()) => "Saved workflow; one-time webhook secret copied".to_string(),
                        Err(error) => format!(
                            "Saved workflow; webhook secret (copy failed: {error}): {secret}"
                        ),
                    },
                    None => "Saved workflow".to_string(),
                };
            }
