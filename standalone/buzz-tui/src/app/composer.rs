use super::{
    nostr_pubkey_uri, parse_attachment_paths, short_id, App, ComposerSubmit, DiffField, Focus,
    Message, PendingMessageSend, TimelineMode,
};
use crate::client::SendDiffOptions;
use buzz_sdk::mentions::{extract_at_mentions_with_known, strip_code_regions};
...
impl App {
    ...
    pub fn accept_completion(&mut self) -> bool {
        ...
        if state.kind == CompletionKind::Mention {
            if let Some(pubkey) = item.mention_pubkey {
                self.composer_mentions.insert(item.insert, pubkey);
            }
        }
        self.save_active_channel_draft();
        true
    ...
    fn completion_matches(&self, kind: CompletionKind, query: &str) -> Vec<CompletionItem> {
        ...
        // Each candidate carries whether it belongs to the active channel so
        // channel members rank above other reachable identities.
        let mut items: Vec<(CompletionItem, bool)> = match kind {
            CompletionKind::Mention => {
                let channel_member_pubkeys = self
                    .channel_members
                    .iter()
                ...
                for agent in self.acp.agents() {
                    if agent.runtime.managed && nostr_pubkey_uri(&agent.runtime.id).is_some() {
                        candidates.push((
                            agent.runtime.label.as_str(),
                            agent.runtime.id.as_str(),
                            "agent",
                        ));
                    }
                }
                for agent in self.available_relay_agents() {
                    if nostr_pubkey_uri(&agent.pubkey).is_some() {
                        candidates.push((agent.name.as_str(), agent.pubkey.as_str(), "relay"));
                    }
                }
                ...
                let mut people = candidates
                    .into_iter()
                    .map(|(label, pubkey, source)| {
                        let duplicate = label_counts
                            .get(&label.to_ascii_lowercase())
                            .copied()
                            .unwrap_or_default()
                            > 1;
                        let display = if duplicate {
                            format!("{label} ({source} {})", short_id(pubkey))
                        } else {
                            format!("{label} ({source})")
                        };
                        (
                            CompletionItem {
                                ...
                                mention_pubkey: Some(pubkey.to_string()),
                            },
                            channel_member_pubkeys.contains(pubkey),
                        )
                    })
                    .collect::<Vec<_>>();
                ...
                for contact in &self.contacts {
                    ...
                    people.push((
                        CompletionItem {
                            display: name.clone(),
                            insert: name,
                            mention_pubkey: Some(contact.pubkey.clone()),
                        },
                        channel_member_pubkeys.contains(contact.pubkey.as_str()),
                    ));
                }
                for member in &self.channel_members {
                    let label = self.author_label(&member.pubkey);
                    people.push((
                        CompletionItem {
                            display: format!("{label} ({})", member.role),
                            insert: label,
                            mention_pubkey: Some(member.pubkey.clone()),
                        },
                        true,
                    ));
                }
        ...
        items.retain(|(item, _)| item.display.to_lowercase().contains(&query));
        items.sort_by(|(a, a_member), (b, b_member)| {
            let a_prefix = a.display.to_lowercase().starts_with(&query);
            let b_prefix = b.display.to_lowercase().starts_with(&query);
            b_prefix
                .cmp(&a_prefix)
                .then(b_member.cmp(a_member))
                .then(a.display.cmp(&b.display))
        });
        let mut items: Vec<CompletionItem> = items.into_iter().map(|(item, _)| item).collect();
        let mut seen_inserts = BTreeSet::new();
        items
            .retain(|item| seen_inserts.insert((item.insert.clone(), item.mention_pubkey.clone())));
        items.truncate(8);
        items
    }
    ...
    pub(super) fn own_pubkey_hex(&self) -> Option<String> {
        self.cli.public_key_hex()
    }
    ...
    pub fn focus_attachment(&mut self) {
        if self.timeline_mode == TimelineMode::Pulse {
            self.status = "Pulse attachments are not supported".to_string();
            return;
        }
    ...
    pub fn focus_diff(&mut self) {
        if self.timeline_mode == TimelineMode::Pulse {
            self.status = "Pulse diffs are not supported".to_string();
            return;
        }
        if self.active_channel().is_none() {
            self.status = "No channel selected".to_string();
    ...
    pub fn prepare_composer_message_send(&mut self) -> ComposerSubmit {
        self.composer_completion = None;
        let content = self.composer.trim().to_string();
        if content.is_empty() {
            return ComposerSubmit::Done;
        }
        if self.edit_target.is_some() || self.timeline_mode == TimelineMode::Pulse {
            return ComposerSubmit::Inline;
        }
        ...
        let surface =
            crate::client::ChannelMessageSurface::from_channel_type(&channel.channel_type);
        let mention_pubkeys = self.composer_mention_pubkeys_for_content(&content);
        let event = match client.build_channel_message_event(
            &channel.id,
            surface,
            &content,
            &mention_pubkeys,
        ...
        let optimistic = Message {
            ...
            thread_root_id: reply_to.clone(),
            edited_at: None,
            edit_event_id: None,
        };
        self.apply_optimistic_sent_message(optimistic);
        self.composer.clear();
        self.composer_mentions.clear();
        self.composer_cursor = 0;
        self.clear_channel_draft(&channel.id);
        self.focus = Focus::Timeline;
        self.status = if reply_to.is_some() {
            format!("Sending reply in #{}", channel.name)
        } else {
            format!("Sending to #{}", channel.name)
        };
        ComposerSubmit::Queued(Box::new(PendingMessageSend {
            client,
            event,
    ...
    fn apply_optimistic_sent_message(&mut self, message: Message) {
        ...
        let changed = super::merge_timeline_messages(&mut self.messages, [message]);
        if !changed {
            return;
        }
    ...
    pub(super) async fn send_composer(&mut self) {
        ...
        let mention_pubkeys = self.composer_mention_pubkeys_for_content(&content);
        let result = self
            .send_message_native(&channel.id, &content, &mention_pubkeys, reply_to.as_deref())
            .await;
        match result {
            Ok(_) => {
                self.composer.clear();
    ...
    pub(super) async fn send_attachment(&mut self) {
        ...
        let reply_to = self.thread_root.clone();
        match self
            .send_message_with_files_native(
                &channel.id,
                &content,
                &mention_pubkeys,
                reply_to.as_deref(),
    ...
    pub(super) fn append_composer_mention(&mut self, label: &str, pubkey: &str) {
        let label = label.trim();
        if !self.composer.is_empty() && !self.composer.ends_with(char::is_whitespace) {
            self.composer.push(' ');
        }
        self.composer.push('@');
        self.composer.push_str(label);
        self.composer.push(' ');
        self.composer_mentions
            .insert(label.to_string(), pubkey.to_string());
        self.composer_cursor = self.composer.len();
        self.save_active_channel_draft();
    }

    pub(super) fn composer_mention_refs_for_content(
        &self,
        content: &str,
    ) -> BTreeMap<String, String> {
        mention_refs_in_content(content, &self.composer_mentions)
    }

    pub(super) fn composer_mention_pubkeys_for_content(&self, content: &str) -> Vec<String> {
        self.composer_mention_refs_for_content(content)
            .into_values()
            .collect()
    }
}
...
pub(super) fn mention_refs_in_content(
    content: &str,
    refs: &BTreeMap<String, String>,
) -> BTreeMap<String, String> {
    let known_names = refs.keys().map(String::as_str).collect::<Vec<_>>();
    let stripped = strip_code_regions(content);
    let names = extract_at_mentions_with_known(&stripped, &known_names)
        .into_iter()
        .collect::<BTreeSet<_>>();
    refs.iter()
        .filter(|(name, _)| names.contains(&name.to_ascii_lowercase()))
...
mod tests {
    ...
    #[test]
    fn mention_refs_follow_visible_names_without_leaking_into_content() {
        let refs = BTreeMap::from([(
            "Helper".to_string(),
            "7e7e9c42a91bfef19fa929e5fda1b72e0ebc1a4c1141673e2794234d86addf4e".to_string(),
        )]);
        assert_eq!(mention_refs_in_content("ask @Helper for help", &refs), refs);
        assert!(mention_refs_in_content("ask Helper for help", &refs).is_empty());
        assert!(mention_refs_in_content("`ask @Helper`", &refs).is_empty());
    }
}
