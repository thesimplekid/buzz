...
fn location_key(entry: &NavigationEntry) -> String {
    ...
    let channel = entry
        ...
        .as_deref()
        .map(str::to_string)
        .unwrap_or_else(|| entry.selected_channel.to_string());
    format!("{panel}|{:?}|{channel}|{thread}", entry.timeline_mode)
}
...
impl App {
    pub(super) fn nav_snapshot(&self) -> NavigationEntry {
        NavigationEntry {
            ...
            active_channel_id: self.active_channel_id.clone(),
    ...
    pub fn track_navigation(&mut self) {
        ...
        if matches!(
            self.focus,
            Focus::CommandPalette | Focus::Pairing | Focus::Confirm
        ) {
            return;
        }
    ...
    async fn apply_nav(&mut self, entry: NavigationEntry) {
        ...
        if entry.timeline_mode == TimelineMode::Channel {
            self.active_channel_id = entry.active_channel_id.clone().or_else(|| {
                self.channels
                    .get(entry.selected_channel)
        ...
        match entry.focus {
            ...
            Focus::Agents => self.focus_agents(),
