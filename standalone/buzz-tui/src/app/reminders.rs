...
impl App {
    pub(super) fn apply_reminder_poll(&mut self, reminders: Vec<Reminder>) {
        let now = now_seconds();
        ...
        let newly_due = due_ids.difference(&self.notified_due_reminders).count();
        self.notified_due_reminders = due_ids;
        self.reminders = reminders;
        let visible_len = self.visible_reminders().len();
        clamp_index(&mut self.selected_reminder, visible_len);
        if newly_due > 0 {
            self.attention_notice = Some(format!(
                "{newly_due} reminder{} due — press L to open reminders",
                if newly_due == 1 { " is" } else { "s are" }
            ));
        }
    }
    ...
    pub async fn open_selected_reminder_thread(&mut self) {
            ...
            .await
        {
            Ok(messages) => {
                ...
                self.begin_unread_session(&target.channel_id, Some(&target.event_id));
