...
use buzz_client::RelayMessage;
use buzz_core::kind::{
    KIND_CONTACT_LIST, KIND_DELETION, KIND_EMOJI_SET, KIND_FORUM_COMMENT, KIND_FORUM_POST,
    KIND_HUDDLE_STARTED, KIND_NIP29_DELETE_EVENT, KIND_NIP29_GROUP_MEMBERS,
    KIND_NIP29_GROUP_METADATA, KIND_PRESENCE_SNAPSHOT, KIND_PRESENCE_UPDATE, KIND_READ_STATE,
    KIND_STREAM_MESSAGE, KIND_STREAM_MESSAGE_DIFF, KIND_STREAM_MESSAGE_EDIT,
    KIND_STREAM_MESSAGE_V2, KIND_SYSTEM_MESSAGE, KIND_TEXT_NOTE, KIND_TYPING_INDICATOR,
};
...
mod tests {
    ...
    fn timeline_message_kind_excludes_non_timeline_events() {
        ...
        assert!(is_timeline_message_kind(KIND_SYSTEM_MESSAGE as u16));
        assert!(is_timeline_message_kind(KIND_HUDDLE_STARTED as u16));
