...
use crate::client::{
    count_due_reminders, group_reminders, huddle_started_description, Channel, ConversationKind,
    ProfileField, SystemMessage,
};

const MIN_TIMELINE_WIDTH: u16 = 30;
const MIN_COMPOSER_HEIGHT: u16 = 4;
const MAX_COMPOSER_HEIGHT: u16 = 10;
const CURSOR_MARKER_COLOR: Color = Color::Rgb(1, 2, 3);

/// A hyperlink fragment to repaint with an explicit terminal destination.
...
pub fn draw(frame: &mut Frame<'_>, app: &App) -> Vec<TerminalHyperlink> {
    let mut terminal_hyperlinks = Vec::new();
    let composer_height = if app.focus == Focus::Composer {
        chat_composer_height(app, frame.area().width)
    } else {
        MIN_COMPOSER_HEIGHT
    };
    ...
    draw_side_panel(frame, app, body[2], &mut terminal_hyperlinks);
...
fn draw_chat_composer(frame: &mut Frame<'_>, app: &App, area: Rect) {
    ...
    let mut lines = chat_composer_context_lines(app);
    ...
    let cursor = composer_cursor_layout(app, area.width.saturating_sub(2));
    let visible_rows = area.height.saturating_sub(2);
    let scroll = cursor
        .map(|position| position.y.saturating_sub(visible_rows.saturating_sub(1)))
        .unwrap_or(0);
    frame.render_widget(
        Paragraph::new(lines)
    ...
    if let Some(position) = cursor {
        let visible_y = position.y.saturating_sub(scroll);
        if visible_y < visible_rows {
            frame.set_cursor_position(Position {
                x: area.x.saturating_add(1).saturating_add(position.x),
                y: area.y.saturating_add(1).saturating_add(visible_y),
            });
        }
    }
}

fn chat_composer_context_lines(app: &App) -> Vec<Line<'static>> {
...
fn chat_composer_height(app: &App, width: u16) -> u16 {
    let inner_width = width.saturating_sub(2);
    if inner_width == 0 {
        return MIN_COMPOSER_HEIGHT;
    }
    let cursor_rows = composer_cursor_layout(app, inner_width)
        .map(|position| position.y.saturating_add(1))
        .unwrap_or(0);
    cursor_rows
        .saturating_add(2)
...
fn composer_cursor_layout(app: &App, inner_width: u16) -> Option<Position> {
    wrapped_cursor_layout(
        &app.composer,
        app.composer_cursor,
        chat_composer_context_lines(app),
        inner_width,
    )
}

fn wrapped_cursor_layout(
    text: &str,
    cursor: usize,
    mut context_lines: Vec<Line<'static>>,
    inner_width: u16,
) -> Option<Position> {
    if inner_width == 0 {
        return None;
    ...
    let cursor = normalized_cursor(text, cursor);
    ...
    let line_end = text[cursor..]
        ...
        .unwrap_or(text.len());
    let cursor_line = text[..line_start].chars().filter(|ch| *ch == '\n').count();
    for (index, line) in text.split('\n').enumerate() {
        if index != cursor_line {
            context_lines.push(Line::from(line.to_string()));
            continue;
        }
        ...
        if cursor < line_end {
            ...
            context_lines.push(Line::from(vec![
                ...
                Span::styled(
                    text[cursor..next_end].to_string(),
                    Style::new().fg(CURSOR_MARKER_COLOR),
                ),
                Span::raw(text[next_end..line_end].to_string()),
            ]));
        } else {
            context_lines.push(Line::from(vec![
                Span::raw(before.to_string()),
                Span::styled(" ", Style::new().fg(CURSOR_MARKER_COLOR)),
            ]));
        }
    }

    // A wrapped line can waste at most roughly half a row when the next word
    // moves down. Twice the hard-wrap count plus one is therefore a safe render
    // bound while avoiding a width-by-character-count scratch allocation.
    let height = context_lines
        .iter()
        .map(|line| {
            let hard_wrapped = line.width().div_ceil(inner_width as usize);
            hard_wrapped.saturating_mul(2).saturating_add(1).max(1)
        })
        .sum::<usize>();
    ...
    for y in 0..height {
        for x in 0..inner_width {
            if buffer[(x, y)].fg == CURSOR_MARKER_COLOR {
...
fn draw_pairing(frame: &mut Frame<'_>, app: &App) {
    use ratatui::widgets::Clear;
    ...
    let qr_width = pairing
        ...
        .max()
        .unwrap_or(0);
    let qr_height = pairing.qr_lines.len() as u16;
    let desired_width = qr_width.saturating_add(6).max(64);
    let desired_height = qr_height.saturating_add(8).max(12);
    let area = centered_rect(desired_width, desired_height, frame.area());
    frame.render_widget(Clear, area);
    let block = panel_block("Pair mobile device", true);
    let inner = block.inner(area);
    frame.render_widget(block, area);

    match pairing.step {
        crate::app::PairingUiStep::Starting => {
            frame.render_widget(
                Paragraph::new(
                    "\nPreparing a secure NIP-AB pairing session…\n\nConnecting to the pairing relay.",
                )
        ...
        crate::app::PairingUiStep::Qr => {
            let fits_qr = qr_width > 0
                && qr_width <= inner.width
                && qr_height.saturating_add(4) <= inner.height;
            if fits_qr {
                let layout = Layout::default()
                    .direction(Direction::Vertical)
                    .constraints([
                    ...
                    .split(inner);
                frame.render_widget(
                    Paragraph::new("Scan with Buzz mobile")
                        .alignment(ratatui::layout::Alignment::Center),
                    layout[0],
                );
                let qr_area = centered_rect(qr_width, qr_height, layout[1]);
                frame.render_widget(
                    Paragraph::new(pairing.qr_lines.join("\n"))
                        .style(Style::new().fg(Color::Black).bg(Color::White)),
                    qr_area,
                );
                frame.render_widget(
                    Paragraph::new("c copy pairing code · Esc cancel")
                        .alignment(ratatui::layout::Alignment::Center)
                ...
                frame.render_widget(
                    Paragraph::new(format!(
                        "This terminal is too small to display the QR code.\n\nCopy this pairing code with c and paste it into Buzz mobile:\n\n{code}\n\nEsc cancels."
                    ))
                    .wrap(Wrap { trim: false }),
                    inner,
                );
        ...
        crate::app::PairingUiStep::Sas => {
            let code = pairing.sas.as_deref().unwrap_or("------");
            ...
            let lines = vec![
                ...
                Line::from(Span::styled(
                    "Verify this code matches Buzz mobile",
                ...
                Line::from(""),
                Line::from("Only confirm if you initiated this pairing."),
                Line::from(""),
                Line::from(vec![
                    Span::styled(
                        "Enter/y codes match",
                    ...
                    Span::raw("    "),
                    Span::styled("n/Esc cancel", Style::new().fg(Color::Red)),
                ]),
            ];
            frame.render_widget(
                Paragraph::new(lines)
        ...
        crate::app::PairingUiStep::Transferring => {
            frame.render_widget(
                Paragraph::new(
                    "\nSecurity code confirmed.\n\nSending the encrypted identity to Buzz mobile…",
                )
                .alignment(ratatui::layout::Alignment::Center)
                .wrap(Wrap { trim: false }),
                inner,
        ...
        crate::app::PairingUiStep::Done => {
            frame.render_widget(
                Paragraph::new(
                    "\nMobile pairing complete.\n\nBuzz mobile confirmed the identity transfer.\n\nEnter or Esc closes.",
                )
                .alignment(ratatui::layout::Alignment::Center)
        ...
        crate::app::PairingUiStep::Error => {
            let error = pairing.error.as_deref().unwrap_or("Pairing failed");
            frame.render_widget(
                Paragraph::new(format!(
                    "\nPairing failed\n\n{error}\n\nEnter or Esc closes."
                ))
...
fn draw_channels(frame: &mut Frame<'_>, app: &App, area: Rect) {
    ...
    let mut items = Vec::with_capacity(channels.len() + 6);
    let mut visual_selected = selected;
    for (index, channel) in channels.iter().enumerate() {
        if app.focus != Focus::ChannelSearch
            && (index == 0
                || sidebar_section_label(app, &channels[index - 1])
                    != sidebar_section_label(app, channel))
        {
        ...
        let label = app.channel_display_label(channel);
        ...
        let section = app
            ...
            .unwrap_or_default();
        let unread = if let Some(count) = app.channel_unread_count(&channel.id) {
            if count > 99 {
                " [99+]".to_string()
            } else {
                format!(" [{count}]")
            }
        } else if app.channel_has_unread(&channel.id) {
            " new".to_string()
        } else {
            String::new()
        };
    ...
    let mut state = list_state(visual_selected, item_count);
    frame.render_stateful_widget(list, area, &mut state);
}

fn sidebar_section_label<'a>(app: &'a App, channel: &'a crate::client::Channel) -> &'a str {
    if channel.archived {
        "Archived"
    } else if app.channel_is_starred(&channel.id) {
        "Starred"
    } else if let Some(section) = app.channel_section_name(&channel.id) {
        section
    } else {
        match channel.kind {
            ConversationKind::DirectMessage => "Direct messages",
            ConversationKind::Channel if channel.channel_type == "forum" => "Forums",
...
fn draw_messages(frame: &mut Frame<'_>, app: &App, area: Rect) {
            ...
            let title = app
                ...
                .map(|channel| {
                    ...
                    if app.thread_root.is_some() {
                        ...
                        match channel.kind {
                            ConversationKind::Channel => format!("#{label}"),
                            ConversationKind::DirectMessage => format!("@{label}"),
                        }
                    }
                })
    ...
    let unread_index = app.first_unread_message_index().filter(|index| *index > 0);
    let items: Vec<ListItem> = messages
        .iter()
        .enumerate()
        .flat_map(|(index, message)| {
            let mut rows = Vec::with_capacity(2);
            if unread_index == Some(index) {
                rows.push(ListItem::new(unread_divider_line(
                    area.width.saturating_sub(2),
                )));
            }
            let system_description =
                SystemMessage::parse(message.kind, &message.content).and_then(|system_message| {
                    system_message.describe(|pubkey| app.author_label(pubkey))
                });
            let huddle_description = huddle_started_description(message.kind, &message.content);
            let author = if system_description.is_some() {
                "system".to_string()
            } else {
                app.author_label(&message.pubkey)
            };
            let content = system_description
                .as_deref()
                .or(huddle_description)
                .unwrap_or(message.content.as_str());
            let timestamp = format_message_time(message.created_at);
            if index == selected {
                ...
                if let Some(first) = lines.first_mut() {
                    insert_thread_badge_before_preview(
                        &mut first.spans,
                        thread_badge_span(app, message),
                        2,
                    );
                }
                ...
                && continues_message_group(&messages[index - 1], message)
            {
                ...
                vec![
                    Span::raw(" ".repeat(author.chars().count() + 2)),
                    Span::styled(format!("{timestamp} "), Style::new().fg(Color::DarkGray)),
                ]
            } else {
                vec![
            ...
            if app.timeline_mode == TimelineMode::Feed
                && index > 0
                && continues_feed_burst(&messages[index - 1], message)
            {
                spans.push(Span::styled(
                    format!("{timestamp} "),
                    Style::new().fg(Color::DarkGray),
            ...
            let preview_start = spans.len();
            insert_thread_badge_before_preview(
                &mut spans,
                thread_badge_span(app, message),
                preview_start,
            );
            spans.extend(crate::render::render_message_preview(content, 120));
    ...
    let visual_selected =
        selected + usize::from(unread_index.is_some_and(|divider_index| selected >= divider_index));
    let item_count = items.len();
...
fn unread_divider_line(width: u16) -> Line<'static> {
    let width = usize::from(width);
    let label = " New ";
    if width <= label.chars().count() {
        return Line::from("─".repeat(width)).style(Style::new().fg(Color::Red));
    }
    let rule_width = width - label.chars().count();
    ...
    Line::from(vec![
        Span::styled("─".repeat(left_width), style),
        Span::styled(label, style),
        Span::styled("─".repeat(right_width), style),
    ])
}
...
fn thread_badge_span(app: &App, message: &crate::client::Message) -> Option<Span<'static>> {
    ...
    let unread = app.thread_has_unread(&message.channel_id, &message.id);
    if reply_count == 0 && !followed {
        return None;
    }
    let (content, style) = thread_badge_content(reply_count, followed, unread);
    Some(Span::styled(content, style))
}

fn thread_badge_content(reply_count: u64, followed: bool, unread: bool) -> (String, Style) {
    let mut parts = Vec::with_capacity(3);
    if followed {
        parts.push("★".to_string());
    }
    if reply_count > 0 {
        parts.push(format!("↳{reply_count}"));
    }
    if unread {
...
fn insert_thread_badge_before_preview(
    ...
    let preview_start = preview_start.min(spans.len());
    spans.insert(preview_start, badge);
    spans.insert(preview_start + 1, Span::raw(" "));
}
...
fn continues_message_group(
    ...
    previous.kind != u64::from(KIND_SYSTEM_MESSAGE)
        && message.kind != u64::from(KIND_SYSTEM_MESSAGE)
        && previous.kind != u64::from(buzz_core::kind::KIND_HUDDLE_STARTED)
        && message.kind != u64::from(buzz_core::kind::KIND_HUDDLE_STARTED)
...
fn draw_side_panel(
    ...
    app: &App,
    area: Rect,
    terminal_hyperlinks: &mut Vec<TerminalHyperlink>,
) {
    if matches!(
        app.focus,
        Focus::Identity
    ...
    if matches!(app.focus, Focus::Timeline | Focus::Feed | Focus::Pulse) {
        draw_message_detail(frame, app, area, terminal_hyperlinks);
        return;
    }
...
fn draw_agents_list(frame: &mut Frame<'_>, app: &App, area: Rect) {
    let agent_items: Vec<ListItem> = app
        ...
        .map(|entry| match entry {
            ...
            AgentDisplayEntry::Entry(AgentListEntry::Relay(agent)) => {
                let available = app.relay_agent_is_available(agent);
                let status = agent
                    .health_status
                    .as_deref()
                    .filter(|status| *status != "healthy")
                    .unwrap_or(&agent.status);
                let source = if agent.managed_by.as_deref() == Some("beekeeper") {
                    "beekeeper"
                } else {
                    "relay"
                };
                ListItem::new(Line::from(vec![
                    ...
                    if available {
                        ...
                        Span::styled(" unavailable", Style::new().fg(Color::DarkGray))
                    },
                ]))
            }
        })
    ...
    let mut state = agent_list_state(app.selected_agent_display_index(), area);
    frame.render_stateful_widget(list, area, &mut state);
}

fn agent_list_state(selected: Option<usize>, area: Rect) -> ListState {
...
fn draw_help(frame: &mut Frame<'_>, area: Rect) {
    let text = [
        ...
        "h: hide selected DM",
        "y: copy selected channel ID to clipboard",
...
fn draw_channel_detail(frame: &mut Frame<'_>, app: &App, area: Rect) {
    ...
    let mut text = format!(
        ...
        if channel.owner_pubkey.trim().is_empty() {
            ...
            app.author_label(&channel.owner_pubkey)
        },
    ...
    if app.channel_members.is_empty() {
        ...
        for member in app.channel_members.iter().take(12) {
            text.push_str(&format!(
                "\n{} {}",
                app.author_label(&member.pubkey),
...
fn channel_id_line(channel: &Channel) -> String {
...
fn draw_message_detail(
    ...
    let system_description = SystemMessage::parse(message.kind, &message.content)
        .and_then(|system_message| system_message.describe(|pubkey| app.author_label(pubkey)));
    let huddle_description = huddle_started_description(message.kind, &message.content);
    let author = if system_description.is_some() {
        "system".to_string()
    } else {
    ...
    let metadata: Vec<Line> = vec![
        Line::from(vec![
            Span::styled("event ", meta),
            Span::raw(short_id(&message.id).to_string()),
        ]),
        Line::from(vec![Span::styled("author ", meta), Span::raw(author)]),
        Line::from(vec![
            Span::styled("sent ", meta),
            Span::raw(format_message_date_time(message.created_at)),
        ]),
        Line::from(vec![
    ...
    let mut lines = crate::render::wrap_message_detail_lines(metadata, detail_width);
    let body_start_row = lines.len();
    let (body_lines, body_hyperlinks) =
        crate::render::render_message_body_with_hyperlinks(content, detail_width);
    ...
    terminal_hyperlinks.extend(body_hyperlinks.into_iter().filter_map(|link| {
        let full_row = body_start_row + link.row;
        let visible_row = full_row.checked_sub(usize::from(scroll))?;
        if visible_row >= visible_rows || link.column >= detail_width {
            return None;
        }
        Some(TerminalHyperlink {
            ...
            width: u16::try_from(link.width).unwrap_or(u16::MAX),
            text: link.text,
            url: link.url,
        })
    }));
        ...
        AgentListEntry::Relay(agent) => {
            ...
            let mut text = format!(
                ...
                if available {
                    ...
                    "relay-announced agent; unavailable to this identity"
                },
                agent.status,
                agent.agent_type,
                agent.pubkey
            ...
            if let Some(managed_by) = agent.managed_by.as_deref() {
                text.push_str(&format!("\nmanaged by: {managed_by}"));
            }
            if let Some(management_mode) = agent.management_mode.as_deref() {
                text.push_str(&format!("\nmanagement: {management_mode}"));
            }
            if let Some(manager_pubkey) = agent.manager_pubkey.as_deref() {
                text.push_str(&format!("\nmanager: {}", short_id(manager_pubkey)));
            }
            if let Some(health_status) = agent.health_status.as_deref() {
                text.push_str(&format!("\nfleet health: {health_status}"));
            }
            if let Some(restarts) = agent.health_restarts {
                text.push_str(&format!("\nrestarts: {restarts}"));
            }
            if let Some(detail) = agent.health_detail.as_deref() {
            ...
            if agent.management_mode.as_deref() == Some("external") {
                text.push_str("\nlifecycle controls: external");
            }
            ...
            frame.render_widget(
                Paragraph::new(text)
                    .wrap(Wrap { trim: false })
                    .block(panel_block("Remote Agent", false)),
                area,
            );
            return;
        }
...
fn local_message_date_time(timestamp: u64) -> Option<chrono::DateTime<Local>> {
    if timestamp == 0 {
        return None;
    }
...
fn format_message_time(timestamp: u64) -> String {
    local_message_date_time(timestamp)
        .map(|date_time| date_time.format("%H:%M").to_string())
        .unwrap_or_else(|| "-".to_string())
}

fn format_message_date_time(timestamp: u64) -> String {
    local_message_date_time(timestamp)
        .map(|date_time| date_time.format("%b %-d, %Y at %H:%M").to_string())
        .unwrap_or_else(|| "-".to_string())
}
...
mod tests {
    use super::{
        agent_list_state, channel_id_line, continues_message_group, extract_memory_refs,
        format_message_date_time, format_message_time, insert_thread_badge_before_preview,
        thread_badge_content, unread_divider_line, wrapped_cursor_layout,
    };
    use crate::client::{Channel, Message};
    use chrono::{Local, TimeZone};
    ...
    #[test]
    fn agent_list_viewport_follows_selection_beyond_visible_rows() {
        let area = Rect::new(0, 0, 30, 7);

        let first = agent_list_state(Some(3), area);
        let later = agent_list_state(Some(23), area);

        assert_eq!(first.selected(), Some(3));
        assert_eq!(first.offset(), 0);
        assert_eq!(later.selected(), Some(23));
        assert_eq!(later.offset(), 19);
    }

    #[test]
    fn composer_cursor_moves_to_soft_wrapped_rows() {
        let text = "abcdefghijklmnopqrstuvwxyz";
        let cursor = wrapped_cursor_layout(text, text.len(), Vec::new(), 10)
    ...
    #[test]
    fn composer_cursor_follows_word_wrapping() {
        let text = "hello abcde";
        let cursor =
            wrapped_cursor_layout(text, 7, Vec::new(), 10).expect("cursor should be visible");
    ...
    #[test]
    fn formats_message_timestamps_in_local_time() {
        let timestamp = Local
            .with_ymd_and_hms(2026, 1, 15, 14, 34, 0)
            .single()
            .expect("valid local test time")
            .timestamp() as u64;

        assert_eq!(format_message_time(timestamp), "14:34");
        assert_eq!(format_message_date_time(timestamp), "Jan 15, 2026 at 14:34");
        assert_eq!(format_message_time(0), "-");
        assert_eq!(format_message_date_time(0), "-");
    }

    #[test]
    fn unread_thread_badge_is_labeled_and_highlighted() {
        let (content, style) = thread_badge_content(3, false, true);

        assert_eq!(content, "↳3 new");
        assert_eq!(style.fg, Some(Color::Yellow));
        assert!(style.add_modifier.contains(Modifier::BOLD));
    }

    #[test]
    fn thread_badge_precedes_long_message_preview() {
        let mut spans = vec![
            Span::raw(" alice "),
            Span::raw("12:34 "),
            Span::raw("a long message preview that will be clipped"),
        ];
        insert_thread_badge_before_preview(&mut spans, Some(Span::raw("↳3 new")), 2);
        let content = spans
            .iter()
        ...
        assert_eq!(
            content,
            " alice 12:34 ↳3 new a long message preview that will be clipped"
        );
        assert!(content
            .chars()
            .take(24)
    ...
    #[test]
    fn unread_divider_fills_the_timeline_width() {
        let line = unread_divider_line(21);
        let content = line
            .spans
            .iter()
            .map(|span| span.content.as_ref())
        ...
        assert_eq!(content.chars().count(), 21);
    ...
    #[test]
    fn channel_id_line_preserves_the_full_id() {
        let channel = Channel {
            id: "9ba26a41-91b9-4c57-83a9-08afd46330d2".to_string(),
            ..Channel::default()
        };

        assert_eq!(
            channel_id_line(&channel),
            "id: 9ba26a41-91b9-4c57-83a9-08afd46330d2"
        );
    }
}
