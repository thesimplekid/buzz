...
/// Sentinel stored in the JSON record when the real nsec lives in the OS
/// keyring instead of the file.
const KEYRING_SENTINEL: &str = "@keyring";
#[cfg(feature = "system-keyring")]
const KEYRING_SERVICE: &str = "buzz-tui-agents";

#[cfg(feature = "system-keyring")]
fn keyring_entry(pubkey: &str) -> Option<keyring::Entry> {
    keyring::Entry::new(KEYRING_SERVICE, pubkey).ok()
}

/// Try to store the agent's nsec in the OS keyring. `false` means the caller
/// should keep the key in the JSON store instead.
#[cfg(feature = "system-keyring")]
fn store_agent_secret(pubkey: &str, nsec: &str) -> bool {
    keyring_entry(pubkey).is_some_and(|entry| entry.set_password(nsec).is_ok())
}

#[cfg(not(feature = "system-keyring"))]
fn store_agent_secret(_pubkey: &str, _nsec: &str) -> bool {
    false
}

#[cfg(feature = "system-keyring")]
fn load_agent_secret(pubkey: &str) -> Option<String> {
    keyring_entry(pubkey).and_then(|entry| entry.get_password().ok())
}

#[cfg(not(feature = "system-keyring"))]
fn load_agent_secret(_pubkey: &str) -> Option<String> {
    None
}

#[cfg(feature = "system-keyring")]
fn delete_agent_secret(pubkey: &str) {
    if let Some(entry) = keyring_entry(pubkey) {
        let _ = entry.delete_credential();
    }
}

#[cfg(not(feature = "system-keyring"))]
fn delete_agent_secret(_pubkey: &str) {}
...
impl ManagedAgentStore {
    ...
    pub fn save(&self, path: &Path) -> Result<(), ManagedAgentStoreError> {
        ...
        write_private_file(path, content.as_bytes()).map_err(ManagedAgentStoreError::Write)
    }
    ...
    pub fn create_agent(
        ...
        // Prefer the OS keyring for the private key, matching the desktop;
        // fall back to the JSON store when no keyring is available.
        let stored_private_key = if store_agent_secret(&pubkey, &private_key_nsec) {
        ...
        let record = TuiManagedAgentRecord {
            ...
            log_path: managed_agent_log_path_for_store(path, &pubkey)
                .display()
                .to_string(),
            private_key_nsec: stored_private_key,
...
impl TuiManagedAgentRecord {
    pub fn info(&self) -> ManagedAgentInfo {
        ManagedAgentInfo {
            ...
            private_key_nsec: if self.private_key_nsec == KEYRING_SENTINEL {
                load_agent_secret(&self.pubkey)
            } else {
                Some(self.private_key_nsec.clone())
            },
...
fn read_log_tail(path: &Path, lines: usize) -> std::io::Result<String> {
    ...
    let mut content = Vec::with_capacity((file_len - start) as usize);
...
mod tests {
    ...
    #[test]
    fn log_tail_bounds_large_file_reads() {
        let dir = tempfile_dir();
        let path = dir.join("large.log");
        let content = (0..10_000)
            .map(|index| format!("log line {index:05} {}", "x".repeat(32)))
            .collect::<Vec<_>>()
            .join("\n");
        ...
        assert!(tail.starts_with("[... earlier log output omitted ...]\n"));
        assert!(tail.ends_with("log line 09999 xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"));
    ...
    fn saved_store_is_owner_only() {
        ...
        let dir = tempfile_dir();
        let path = dir.join("agents.json");
        ManagedAgentStore::default().save(&path).unwrap();
        assert_eq!(
